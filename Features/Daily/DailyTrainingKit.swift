...
// MARK: - Types (präfixiert)

enum DTWeekday: Int, CaseIterable {
    case sunday = 1, monday, tuesday, wednesday, thursday, friday, saturday

    static func from(_ date: Date) -> DTWeekday {
        let cal = Calendar(identifier: .gregorian)
        let tz = TimeZone(identifier: "Europe/Zurich") ?? .current
        let weekdayNumber = cal.component(.weekday, from: date)
        return DTWeekday(rawValue: weekdayNumber) ?? .monday
    }
...
struct DTExercise: Identifiable {
    let id = UUID()
    let name: String
    let setsReps: String
}
...
struct DTSession {
    let day: DTWeekday
    let title: String
    let exercises: [DTExercise]
}

// MARK: - Planner mit deinen Inhalten

final class DTPlanner: ObservableObject {
    private let sessions: [DTWeekday: DTSession] = [
        .monday: DTSession(
            day: .monday,
        ...
        .friday: DTSession(
            ...
            title: "Upper Volumen",
            exercises: [
                .init(name: "Incline Benchpress",    setsReps: "4×12–15"),
    ...
    func session(for date: Date = Date()) -> DTSession? {
            sessions[DTWeekday.from(date)]
        }
}

// MARK: - Minimalistisches Popup

struct DTTrainingPopupView: View {
    let session: DTSession
...
struct DTTrainingCard: View {
    @StateObject private var planner = DTPlanner()
